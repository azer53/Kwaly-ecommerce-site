...
function Cart(props) {
    ...
    return (
        <div className={`${state.cart.length > 0 ? `block` : `hidden`} absolute md:w-2/6 lg:w-1/6 sm:right-auto right-0 mt-12 bg-gray-200 shadow-lg p-4 sm:-ml-12 sm:mt-10 lg:mt-6 rounded-sm z-50`}>
            ...
            {state.cart.map((item) => {
                return (
                    <div className="border-b-2 pb-2 mt-2" key={item.id + '-' + item.selectedSize}>
                        <div>
                        ...
                        <button className="text-xs italic hover:underline" onClick={() => { dispatch({ type: "DELETE_FROM_CART", value: item }) }}> Remove item from cart </button>
                    </div>
                )
            })}
