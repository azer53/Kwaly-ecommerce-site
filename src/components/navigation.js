            ...
            <nav className="toggleColour" id="header">
                ...
                <div id="nav-content" className="w-full block flex-grow lg:flex lg:items-center lg:w-auto mt-8">
                    <div className="text-sm lg:flex-grow flex justify-center">
                        <Link to={`/shop`} activeClassName="underline" href="#responsive-header" className="block mt-4 lg:inline-block lg:mt-0 text-gray-800 hover:text-gray-500 mx-3 lg:mx-4">
                            SHOP
      </Link>
                        <Link to={`/brand`} activeClassName="underline" className="block mt-4 lg:inline-block lg:mt-0 text-gray-800 hover:text-gray-500 mx-3 lg:mx-4">
                            BRAND
      </Link>
                        <Link to={`/page-2`} activeClassName="underline" className="block mt-4 lg:inline-block lg:mt-0 text-gray-800 hover:text-gray-500 mx-3 lg:mx-4">
                            SOCIAL
      </Link>
                        <button onClick={() => setIsCartExpanded(!isCartExpanded)} className="block mt-4 lg:inline-block lg:mt-0 text-gray-800 hover:text-gray-500 mx-3 lg:mx-4 outline-none">CART<span className={`${state.cart.length > 0 ? `inline` : `hidden`} rounded-full bg-indigo-200 uppercase px-2 py-1 text-xs font-bold ml-1`}>{state.cart.totalItems}</span></button>
